...
#endif // _DEBUG
TSTRING										strShaderFileName = _T("Shader_Load_Image.hlsl");
...
HRESULT Create_D3D12_Root_Signature() {
	...
	//RootSignature Desc1
	{
		...
		D3D12_ROOT_PARAMETER1 stD3D12RootParameter1s[2] = {};
		{
			D3D12_DESCRIPTOR_RANGE1 stD3D12CBVSRVRange1s[2] = {}; 
			{
				{
					stD3D12CBVSRVRange1s[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
					stD3D12CBVSRVRange1s[0].NumDescriptors = 1;
					stD3D12CBVSRVRange1s[0].BaseShaderRegister = 0;
					stD3D12CBVSRVRange1s[0].RegisterSpace = 0;
					stD3D12CBVSRVRange1s[0].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC;
					stD3D12CBVSRVRange1s[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
				}
				{
					stD3D12CBVSRVRange1s[1].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
					stD3D12CBVSRVRange1s[1].NumDescriptors = 1;
					stD3D12CBVSRVRange1s[1].BaseShaderRegister = 0;
					stD3D12CBVSRVRange1s[1].RegisterSpace = 0;
					stD3D12CBVSRVRange1s[1].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC;
					stD3D12CBVSRVRange1s[1].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
				}
				stD3D12RootParameter1s[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
				stD3D12RootParameter1s[0].DescriptorTable.NumDescriptorRanges = _countof(stD3D12CBVSRVRange1s);
				stD3D12RootParameter1s[0].DescriptorTable.pDescriptorRanges = &stD3D12CBVSRVRange1s[0];
			...
			}
			{
				...
				stD3D12RootParameter1s[1].DescriptorTable.NumDescriptorRanges = _countof(stD3D12SamplerRangels);
				stD3D12RootParameter1s[1].DescriptorTable.pDescriptorRanges = &stD3D12SamplerRangels[0];
				stD3D12RootParameter1s[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
			}
		}
		//Static Sampler Desc
...
HRESULT Populate_CommandList() {
	...
	//Set CBVSRV Heap
	ID3D12DescriptorHeap* ppCBVSRVHeaps[] = { pID3D12CBVSRVHeap.Get() };
	pID3D12GraphicsCommandList->SetDescriptorHeaps(_countof(ppCBVSRVHeaps), ppCBVSRVHeaps);
	pID3D12GraphicsCommandList->SetGraphicsRootDescriptorTable(0, pID3D12CBVSRVHeap->GetGPUDescriptorHandleForHeapStart());
	//Set Sampler Heap
	...
	ID3D12DescriptorHeap* ppSamplerHeaps[] = { pID3D12SamplerHeap.Get() };
	pID3D12GraphicsCommandList->SetDescriptorHeaps(_countof(ppSamplerHeaps), ppSamplerHeaps);
	pID3D12GraphicsCommandList->SetGraphicsRootDescriptorTable(1, stD3D12GPUSamplerHandle);
