...
_Check_return_ HRESULT Create_D3D12_Fence(_In_ ID3D12Device4* pID3D12Device4, _In_ UINT64 nD3D12FenceCPUValue, _COM_Outptr_ ID3D12Fence** ppID3D12Fence,_In_ D3D12_FENCE_FLAGS enD3D12FenceFlags=D3D12_FENCE_FLAG_NONE) {
	RETURN_IF_FAILED(pID3D12Device4->CreateFence(nD3D12FenceCPUValue, enD3D12FenceFlags, IID_PPV_ARGS(ppID3D12Fence)));
	return S_OK;
}
...
_Check_return_ HRESULT Create_D3D12_RTVs(_In_ ID3D12DescriptorHeap* pID3D12RTVHeap, _In_ IDXGISwapChain3* pIDXGISwapChain3, _Out_ ComPtr<ID3D12Resource> comID3D12RenderTargetResources[]) {
	...
	for (UINT Index = 0; Index < pID3D12RTVHeap->GetDesc().NumDescriptors; ++Index) {

		RETURN_IF_FAILED(pIDXGISwapChain3->GetBuffer(Index, IID_PPV_ARGS(&comID3D12RenderTargetResources[Index])));
		pID3D12Device4->CreateRenderTargetView(comID3D12RenderTargetResources[Index].Get(), nullptr, stD3D12CPURTVHandle);
...
_Check_return_ HRESULT Create_D3D12_RTVs(_In_ ID3D12DescriptorHeap* pID3D12RTVHeap, _In_ D3D12_RESOURCE_DESC *pstD3D12ResourceDesc, _Out_ ComPtr<ID3D12Resource> comID3D12RenderTargetResources[]) {
	...
	for (UINT Index = 0; Index < pID3D12RTVHeap->GetDesc().NumDescriptors; ++Index) {
		...
		RETURN_IF_FAILED(pID3D12Device4->CreateCommittedResource(&stD3D12HeapProperties, D3D12_HEAP_FLAG_NONE, pstD3D12ResourceDesc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&comID3D12RenderTargetResources[Index])));
		pID3D12Device4->CreateRenderTargetView(comID3D12RenderTargetResources[Index].Get(), nullptr, stD3D12CPURTVHandle);
...
const UINT										nBackBufferNum = 3;
struct ST_GPU_PARAMS
{
	UINT										m_nIndex;
	ComPtr<ID3D12Device4>						m_pID3D12Device4;
	ComPtr<ID3D12CommandQueue>					m_pID3D12DirectCommandQueue;

	const UINT									m_nRTVNum = nBackBufferNum;
	ComPtr<ID3D12DescriptorHeap>				m_pID3D12RTVHeap;
	UINT										m_nRTVHandleSize = 0;
	ComPtr<ID3D12Resource>						m_pID3D12RenderTargetResources[nBackBufferNum];

	const UINT									m_nDSVNum = 1;
	ComPtr<ID3D12DescriptorHeap>				m_pID3D12DSVHeap;
	UINT										m_nDSVHandleSize = 0;
	ComPtr<ID3D12Resource>						m_pID3D12DepthStencilResources[1];

	ComPtr<ID3D12CommandAllocator>				m_pID3D12PreDirectCommandAllocator[nBackBufferNum];
	ComPtr<ID3D12GraphicsCommandList>			m_pID3D12DirectGraphicsCommandList[nBackBufferNum];

	ComPtr<ID3D12RootSignature>					m_pID3D12RootSignature;
	ComPtr<ID3D12PipelineState>					m_pID3D12PipelineState;

	ComPtr<ID3D12Fence>							m_pID3D12Fence;

	UINT									m_nszSRVCBVUAV;

	
	ComPtr<ID3D12DescriptorHeap>			m_pIDHRTV;
	ComPtr<ID3D12Resource>					m_pIRTRes[nBackBufferNum];
	ComPtr<ID3D12GraphicsCommandList>	m_pICmdList;
	ComPtr<ID3D12Heap>					m_pICrossAdapterHeap;
	ComPtr<ID3D12Resource>				m_pICrossAdapterResPerFrame[nBackBufferNum];
...
const UINT									nGPUNum = 2;
enum EN_GPU_NAME							{ enIntegrationGPU = 0, enExternalGPU = 1 };
...
int APIENTRY WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
	...
	try {
		...
		//Load Pipeline--------------------------------------------------------------------------------------------
		{
			...
			//ID3D12Device And Command Queuue
			for (UINT IndexGPU = 0; IndexGPU < nGPUNum; ++IndexGPU) {
				stGPUParams[IndexGPU].m_nIndex = IndexGPU;

				IDXGIAdapter1* pIDXGIAdapter1 = nullptr;

				THROW_IF_FAILED(pIDXGIFactory6->EnumAdapterByGpuPreference(0, DXGI_GPU_PREFERENCE_MINIMUM_POWER, IID_PPV_ARGS(&pIDXGIAdapter1)));
				THROW_IF_FAILED(Create_D3D12_Device(pIDXGIAdapter1, D3D_FEATURE_LEVEL_12_1, &stGPUParams[IndexGPU].m_pID3D12Device4));

				THROW_IF_FAILED(Create_D3D12_Direct_Command_Queue(g_pID3D12Device4.Get(), &stGPUParams[IndexGPU].m_pID3D12DirectCommandQueue));
			}

			//Swap Chain
			THROW_IF_FAILED(Create_DXGI_SwapChain(pIDXGIFactory6.Get(), enDXGIRenderTargetormat, nClientWidth, nClientHeight, nBackBufferNum, stGPUParams[enIntegrationGPU].m_pID3D12DirectCommandQueue.Get(), hWnd, pIDXGISwapChain3));
			THROW_IF_FAILED(pIDXGIFactory6->MakeWindowAssociation(hWnd, DXGI_MWA_NO_ALT_ENTER));

			//RTV And DSV Heap
			for (UINT IndexGPU = 0; IndexGPU < nGPUNum; ++IndexGPU) {
				//RTV Heap
				THROW_IF_FAILED(Create_D3D12_Descriptor_Heap(stGPUParams[IndexGPU].m_pID3D12Device4.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV, stGPUParams[IndexGPU].m_nRTVNum, &stGPUParams[IndexGPU].m_pID3D12RTVHeap));
				stGPUParams[IndexGPU].m_nRTVHandleSize = stGPUParams[IndexGPU].m_pID3D12Device4->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

				//DSV Heap
				THROW_IF_FAILED(Create_D3D12_Descriptor_Heap(stGPUParams[IndexGPU].m_pID3D12Device4.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV, stGPUParams[IndexGPU].m_nDSVNum, &stGPUParams[IndexGPU].m_pID3D12DSVHeap));
				stGPUParams[IndexGPU].m_nDSVHandleSize = stGPUParams[IndexGPU].m_pID3D12Device4->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
			}

			//Direct Command List
			for (UINT IndexGPU = 0; IndexGPU < nGPUNum; ++IndexGPU)
				for (UINT IndexBackBuffer = 0; IndexBackBuffer < nBackBufferNum; ++IndexBackBuffer) {
					THROW_IF_FAILED(Create_D3D12_Command_Allocator(stGPUParams[IndexGPU].m_pID3D12Device4.Get(), D3D12_COMMAND_LIST_TYPE_DIRECT, &stGPUParams[IndexGPU].m_pID3D12PreDirectCommandAllocator[IndexBackBuffer]));
					THROW_IF_FAILED(Create_D3D12_Graphics_Command_List(stGPUParams[IndexGPU].m_pID3D12PreDirectCommandAllocator[nBackBufferNum].Get(), D3D12_COMMAND_LIST_TYPE_DIRECT, &stGPUParams[IndexGPU].m_pID3D12DirectGraphicsCommandList[nBackBufferNum]));
				}
		}

		//Load Assert----------------------------------------------------------------------------------------------
		{
			//Error Root Signature
			THROW_IF_FAILED(Create_D3D12_Root_Signature(stGPUParams[enIntegrationGPU].m_pID3D12Device4.Get(), &stGPUParams[enIntegrationGPU].m_pID3D12RootSignature));
			THROW_IF_FAILED(Create_D3D12_Root_Signature(stGPUParams[enExternalGPU].m_pID3D12Device4.Get(), &stGPUParams[enExternalGPU].m_pID3D12RootSignature));
			...
			//Pipeline State
			{
				...
				THROW_IF_FAILED(Create_D3D12_Pipeline_State(stGPUParams[enIntegrationGPU].m_pID3D12RootSignature.Get(), &wstrShaderFileName, &stD3D12BlendDesc, &stD3D12RasterizerDesc, &stD3D12DepthStencilDesc, &stD3D12InputDesc, enDXGIRenderTargetormat, enDXGIDepthStencilFormat, &stGPUParams[enIntegrationGPU].m_pID3D12PipelineState));

				stD3D12DepthStencilDesc.DepthEnable = TRUE;
				THROW_IF_FAILED(Create_D3D12_Pipeline_State(stGPUParams[enExternalGPU].m_pID3D12RootSignature.Get(), &wstrShaderFileName, &stD3D12BlendDesc, &stD3D12RasterizerDesc, &stD3D12DepthStencilDesc, &stD3D12InputDesc, enDXGIRenderTargetormat, enDXGIDepthStencilFormat, &stGPUParams[enExternalGPU].m_pID3D12PipelineState));
			}

			//Fence
			{
				THROW_IF_FAILED(Create_D3D12_Fence(stGPUParams[enIntegrationGPU].m_pID3D12Device4.Get(), nFenceCPUValue++, &pID3D12Fence, D3D12_FENCE_FLAG_SHARED | D3D12_FENCE_FLAG_SHARED_CROSS_ADAPTER));

				HANDLE hShaderFence = nullptr;
				THROW_IF_FAILED(stGPUParams[enIntegrationGPU].m_pID3D12Device4->CreateSharedHandle(stGPUParams[enIntegrationGPU].m_pID3D12Fence.Get(), nullptr, GENERIC_ALL, nullptr, &hShaderFence));
				THROW_IF_FAILED(stGPUParams[enExternalGPU].m_pID3D12Device4->OpenSharedHandle(hShaderFence, IID_PPV_ARGS(&stGPUParams[enExternalGPU].m_pIFence)));
				::CloseHandle(hShaderFence);
			...
			//RTV
			{
				THROW_IF_FAILED(Create_D3D12_RTVs(stGPUParams[enIntegrationGPU].m_pID3D12RTVHeap.Get(), pIDXGISwapChain3.Get(), stGPUParams[enIntegrationGPU].m_pID3D12RenderTargetResources));

				DXGI_SWAP_CHAIN_DESC stDXGISwapChainDesc = {};
				pIDXGISwapChain3->GetDesc(&stDXGISwapChainDesc);
				D3D12_RESOURCE_DESC stD3D12RenderTargetResourceDesc = Fill_D3D12_Resource_Desc_Tex2D(stDXGISwapChainDesc.BufferDesc.Format, stDXGISwapChainDesc.BufferDesc.Width, stDXGISwapChainDesc.BufferDesc.Height, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET);

				THROW_IF_FAILED(Create_D3D12_RTVs(stGPUParams[enExternalGPU].m_pID3D12RTVHeap.Get(), &stD3D12RenderTargetResourceDesc, stGPUParams[enIntegrationGPU].m_pID3D12RenderTargetResources));
			}

			//DSV
			for (UINT IndexGPU = 0; IndexGPU < nGPUNum; ++IndexGPU)
				THROW_IF_FAILED(Create_D3D12_DSVs(stGPUParams[IndexGPU].m_pID3D12DSVHeap.Get(), enDXGIDepthStencilFormat, nClientWidth, nClientHeight, stGPUParams[IndexGPU].m_pID3D12DepthStencilResources));
		}
